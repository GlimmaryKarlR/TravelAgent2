...
export default function ItineraryHub({ demoTrips = [] }: { demoTrips?: any[] }) {
  ...
  return (
    <div className="p-6 space-y-10 h-full overflow-y-auto no-scrollbar py-10 pb-32">
      <header>
        <h1 className="font-serif italic text-4xl font-bold mb-1 tracking-tight">Your Expedition</h1>
        <p className="text-white/40 text-[10px] uppercase font-bold tracking-widest">
          {latestTrip ? `${latestTrip.destination} • ${latestTrip.dates || 'Active'}` : 'No active trips'}
        </p>
      </header>
            ...
            >
                  ...
                  {intelligence.flights?.map((f: any, i: number) => (
                    <div key={i} className="glass-panel p-5 border-white/10! space-y-4 hover:bg-white/[0.03] transition-colors">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button className="px-3 py-2 rounded-lg bg-white/5 text-white/20 text-[9px] uppercase font-bold tracking-widest">Manual</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                ...
                {intelligence.hotels?.map((h: any, i: number) => (
                  <div key={i} className="glass-panel p-6 border-white/10! flex flex-col md:flex-row gap-6 items-start md:items-center bg-gradient-to-r from-white/[0.02] to-transparent">
                    <div className="flex-1 space-y-2">
                      <div className="flex items-baseline gap-3">
                        <h4 className="text-lg font-serif italic">{h.name}</h4>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {intelligence.tours?.map((t: any, i: number) => (
                    <div key={i} className="glass-panel p-4 border-white/10! space-y-3">
                      ...
                      <button 
                        ...
                        className="w-full py-2 bg-gold/5 border border-gold/20 text-gold text-[8px] font-bold uppercase tracking-widest rounded-lg"
                      >
